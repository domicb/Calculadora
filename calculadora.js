/*********************************
 ...
 Fecha creaci贸n: 24/10/2015
 脷ltima modificaci贸n: 08/11/2015
 Versi贸n: x.xx
 ***********************************/
...
function fil(e)
{
    ...
    validos = '0123456789'
    //a帽adimos especiales para poder usar back space y cursor
...
function operacion(signo)
{
    var anterior = document.fo.entrada.value;
    if (anterior == "")
    {//si est谩 vacio lo dejamos tal cual
...
function resumen(emp)
{
    for (var i = emp; i < agenda.length; i++) {
        ...
        td.appendChild(id); // A帽adimos al td el id
        tr.appendChild(td); //A帽adimos al tr el td
        ...
        for (var j = 0; j < 4; j++) { //Recorre cada Contacto
            ...
            // A帽adir el nodo Text como hijo del nodo 	Element 
            td.appendChild(contenido);
            tr.appendChild(td);
            // A帽adir el nodo Element como hijo de la pagina 
...
function mostrarContacto()
{
    ...
    if (pos >= 0 && pos < agenda.length) {//existe la posici贸n en el array
        ...
        alert("El contacto introducido no tiene una referencia v谩lida");
    }
}
...
{
    contactos = new Array('Cristina', 'Nu帽ez Gil', '666333777', '11/09/1993');
    guardar(contactos);
...
//elimina el registro pasado por par醡etro
...
{
    var pos = cont - 1;//en el array es una posici贸n menos
    var contacto = new Array(4);
    contacto = agenda[pos];//recuperamos el contacto del array
...
function menos()
{
    var pos = document.getElementById('inicio').innerHTML - 1;//posici贸n anterior
...
//funcion que a馻de un nuevo contacto
function nuevo()
{
